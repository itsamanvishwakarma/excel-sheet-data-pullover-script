...
def update_secondary_sheet():
    # Load data from both sheets, starting from row 3 (skiprows=2)
    ...
    # Add the actual Excel row numbers (adding 3 because we skipped 2 rows)
    ...
    secondary_df = pd.read_excel(
        ...
        skiprows=2,
        usecols=[2, 3, 4, 17]  # Same columns as primary
    )
    ...
    # Create a dictionary of primary data for updating, including row numbers
    update_dict = {}
    ...
    # Update Physical Count where matches exist
    for _, row in secondary_df.iterrows():
        ...
        if match_key in update_dict:
            ...
            # Update the cell in column Q (17)
